"""
Stratified VPD vs pump Analysis by Antecedent Rainfall

Creates a 2x2 plot showing VPD vs pumps for each rain category,
...
def load_and_merge_data() -> pd.DataFrame:
    """Load weather-pump CSVs and merge with rain data."""
    
    # Load weather-pump data
    dfs = []
    for year in YEARS:
    ...
    if not dfs:
        raise RuntimeError("No weather-pump datasets found.")
...
def plot_stratified(df: pd.DataFrame, vpd_col: str = "VPD_max"):
    """Create 2x2 stratified VPD vs pumps plot."""
    ...
    pump_max = df_valid["pump_hours"].quantile(0.99)
    ...
    fig.suptitle(
        f"VPD vs Pumps by Antecedent Rainfall (threshold: {SIGNIFICANT_RAIN_THRESHOLD} mm)",
        fontsize=13,
        fontweight="bold",
        y=0.92
    )
    ...
    out_path = out_dir / f"pumps_vs_{vpd_col}_by_rain_category.png"
...
def main():
    ...
    out_dir = BASE_DIR / "4_vpd_max_rain_studies"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "daily_weather_pump_rain_merged.csv"
