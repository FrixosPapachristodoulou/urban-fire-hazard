...
def plot_fires_vs_vpd(df: pd.DataFrame):
    ...
    print(
        "\nBest generalised exponential fit on SPRING/SUMMER only (NoF+1):\n"
        f"  ln(NoF + 1) = ln({a_exp:.3g}) + {b_exp:.3e} * VPD^{c_exp:.3f}\n"
        f"  Equivalent: NoF = {a_exp:.3g} * exp({b_exp:.3e} * VPD^{c_exp:.3f}) - 1\n"
    ...
    # NoF = a * exp(b * VPD^c) - 1 (using best c)
    ...
    # generalised exponential fit curve (black), fitted on spring/summer only
    ax.plot(
        vpd_fit,
        fires_fit,
        color="black",
    ...
    glm_label = (
        r"$\ln(\mathrm{NoF}+1)"
        rf" = \ln({a_exp:.2g})"
        rf" + {b_exp:.2e}\,\mathrm{{VPD}}^{{{c_exp:.2f}}}$"
        "\n"
...
def main():
    ...
    print("Plotting fire_count vs VPD_mean with generalised exponential fit...")
