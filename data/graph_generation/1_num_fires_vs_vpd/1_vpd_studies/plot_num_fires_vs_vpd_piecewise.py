...
def fit_single_quadratic(vpd: np.ndarray, fires: np.ndarray):
    ...
    # Also compute regime-specific R²
...
def plot_fires_vs_vpd(df: pd.DataFrame):
    ...
    print(f"   Low VPD: ln(NoF+1) = {pw_unc['low']['alpha']:.2f} + {pw_unc['low']['beta']:.2f}·ln(VPD), R² = {pw_unc['low']['r2']:.3f}")
    print(f"   High VPD: ln(NoF+1) = {pw_unc['high']['alpha']:.2f} + {pw_unc['high']['beta']:.2f}·ln(VPD), R² = {pw_unc['high']['r2']:.3f}")
