...
def analyze_thresholds(df: pd.DataFrame):
    """Analyze R² at different fire count thresholds (using VPD_max)."""
    ...
    print("\n" + "=" * 80)
    print("FIRE COUNT DISTRIBUTION (spring/summer, VPD_max)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("R² BY FIRE COUNT THRESHOLD (spring/summer, VPD_max)")
    print("=" * 80)
...
def plot_diagnostic(df: pd.DataFrame):
    """Create diagnostic plots to understand the fitting problem (VPD_max)."""
    df["season"] = df["met_day"].apply(
        lambda d: "summer" if is_spring_summer(d) else "winter"
    )
    summer_df = df[df["season"] == "summer"]
    ...
    ax2.set_title("Predicted vs Actual (spring/summer, VPD_max fit)")
    ax2.legend()
    ...
    ax3.set_title("Distribution of Daily Fire Counts (spring/summer)")
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    
    # === Plot 4: VPD_max distribution for different fire levels ===
    ax4 = axes[1, 1]
    ...
    ax4.set_xlabel("VPD_max (Pa)")
    ax4.set_ylabel("Frequency (days)")
    ax4.set_title("VPD_max Distribution by Fire Count Level (spring/summer)")
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    
    out_path = BASE_DIR / "2_vpd_max_studies/fires_vs_vpd_max_diagnostic.png"
    ...
    # === Key insight ===
    print("\n" + "=" * 80)
    print("KEY INSIGHT (using VPD_max)")
    print("=" * 80)
    
    # Check VPD_max overlap between fire levels
    low_fire_vpd = x[y <= 15]
    high_fire_vpd = x[y > 30]
    
    print(f"\nVPD_max range for low-fire days (≤15): {np.min(low_fire_vpd):.0f} - {np.max(low_fire_vpd):.0f} Pa")
    print(f"VPD_max range for high-fire days (>30): {np.min(high_fire_vpd):.0f} - {np.max(high_fire_vpd):.0f} Pa")
    print(f"\nVPD_max median for low-fire days: {np.median(low_fire_vpd):.0f} Pa")
    print(f"VPD_max median for high-fire days: {np.median(high_fire_vpd):.0f} Pa")
    
    # Overlap
    overlap_min = max(np.min(low_fire_vpd), np.min(high_fire_vpd))
    overlap_max = min(np.max(low_fire_vpd), np.max(high_fire_vpd))
    print(f"\nOVERLAP ZONE (VPD_max): {overlap_min:.0f} - {overlap_max:.0f} Pa")
    print("→ Same VPD_max values can produce BOTH low and high fire counts!")
    print("→ This is why VPD_max alone cannot reliably predict high-fire days.")


def main():
    print("Loading data...")
    df = load_all_data()
    print("Running diagnostic analysis (VPD_max)...")
    plot_diagnostic(df)
