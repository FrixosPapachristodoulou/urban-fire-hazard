...
def plot_comparison(df: pd.DataFrame):
    """Compare all fitting approaches (using VPD_max)."""
    ...
    # Use VPD_max instead of VPD_mean
    vpd = summer_df["VPD_max"].values
    fires = summer_df["fire_count"].values.astype(float)

    print("\n" + "=" * 80)
    print("COMPARING FIRE-COUNT WEIGHTING STRATEGIES (VPD_max)")
    ...
    r2_all, r2_low, r2_high, fire_threshold = compute_r2_by_fire_regime(
        vpd, fires, alpha, beta, gamma
    )
    ...
    print(f"\n8. COMBINED WEIGHT (VPD_max^0.5 × fire^1):")
    print(f"   R² overall={r2_all:.3f}, low-fire={r2_low:.3f}, HIGH-FIRE={r2_high:.3f}")
    ...
    # Scatter points - color by fire count intensity (use VPD_max for both seasons)
    ax.scatter(winter_df["VPD_max"], winter_df["fire_count"],
               color="blue", alpha=0.50, s=20, label="Autumn/Winter")
    ax.scatter(summer_df["VPD_max"], summer_df["fire_count"],
               color="red", alpha=0.60, s=20, label="Spring/Summer")
    
    # Highlight high-fire days (on summer subset used for fitting)
    ...
    ax.set_title(
        "Daily Wildfires vs VPD_max (London, 2009–2024)\n"
        "Optimized for High-Fire Day Prediction",
        fontsize=14
    )
    ...
    textstr = (f"Best fit for high-fire days ({best_high_fire[0]}):\n"
               f"ln(NoF+1) = {best[0]:.2f} + {best[1]:.2f}·ln(VPD_max)"
               f" + {best[2]:.3f}·[ln(VPD_max)]²\n"
    ...
    out_path = BASE_DIR / "2_vpd_max_studies/fires_vs_vpd_max_fire_weighted.png"
