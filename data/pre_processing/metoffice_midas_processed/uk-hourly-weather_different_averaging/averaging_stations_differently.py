...
INPUT_ROOT = BASE_DIR / "2_daily_by_year"      # per-station daily (with VPD_mean already computed)
OUTPUT_ROOT = BASE_DIR / "3_averaged_by_year"  # station-averaged daily
...
def average_daily_across_stations(year: int):
    ...
    # We want to use 'id' only for station_count, not for averaging
    ...
    # This includes VPD_mean (now already computed per station in step 2_daily_by_year)
    ...
    # 🔹 Round all float columns (including network VPD_mean) to 3 decimal places
    float_cols = daily_out.select_dtypes(include="float").columns
    daily_out[float_cols] = daily_out[float_cols].round(3)

    # Ensure met_day is written as YYYY-MM-DD (no time)
