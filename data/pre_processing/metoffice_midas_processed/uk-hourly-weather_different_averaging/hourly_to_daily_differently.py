...
BASE_DIR = Path("data/pre_processing/metoffice_midas_processed/uk-hourly-weather_different_averaging")
...
def aggregate_hourly_to_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- Compute daily VPD_mean from daily mean T and RH ---
    if "air_temperature_mean" in daily_main.columns and "rltv_hum_mean" in daily_main.columns:
        T_daily = daily_main["air_temperature_mean"]
        RH_daily = daily_main["rltv_hum_mean"].clip(0, 100)

        esat_daily = 610.7 * (10 ** (7.5 * T_daily / (237.3 + T_daily)))
        phi_daily = RH_daily / 100.0

        # Final daily VPD, rounded to 3 decimals
        daily_main["VPD_mean"] = (esat_daily * (1 - phi_daily)).round(3)
    else:
        daily_main["VPD_mean"] = np.nan
    # -------------------------------------------------------
    ...
    # Move VPD_mean to just after met_day
    if "VPD_mean" in daily_main.columns:
        vpd = daily_main["VPD_mean"]
        daily_main = daily_main.drop(columns=["VPD_mean"])
        insert_idx = daily_main.columns.get_loc("met_day") + 1
        daily_main.insert(insert_idx, "VPD_mean", vpd)
